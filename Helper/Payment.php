...
class Payment
{
    ...
    const CC_REGISTER = 'payzen_cc_register'; // key to save if card data register is on
    const MULTI_OPTION = 'payzen_multi_option'; // key to save choosen multi option
    const ONEY_OPTION = 'payzen_oney_option'; // key to save choosen Oney option
     ...
     */
    public function registerOrder(\Magento\Sales\Model\Order $order, \Lyranetwork\Payzen\Model\Api\PayzenResponse $response)
    {
    ...
    public function nextOrderState(
        ...
        if ($response->isToValidatePayment()) {
            ...
            $processingStatuses = $this->orderConfig
                ->getStateStatuses(\Magento\Sales\Model\Order::STATE_PROCESSING, false);
            $newState = in_array($newStatus, $processingStatuses) ?
                \Magento\Sales\Model\Order::STATE_PROCESSING : \Magento\Sales\Model\Order::STATE_NEW;
        }
    ...
    public function updatePaymentInfo(
        ...
        $order->getPayment()->setCcTransId($response->get('trans_id'))
                            ...
                            ->setAdditionalInformation(
                                \Lyranetwork\Payzen\Helper\Payment::ALL_RESULTS,
                                serialize($response->getAllResults())
                            )
                            ->setAdditionalInformation(
                                \Lyranetwork\Payzen\Helper\Payment::TRANS_UUID,
                                $response->get('trans_uuid')
                            );
        ...
        if ($response->get('card_brand') == 'MULTI') { // multi brand
            ...
            foreach ($transactions as $trs) {
                ...
                $additionalInfo = [
                    ...
                    'Extra Transaction ID' => property_exists($trs, 'ext_trans_id') && isset($trs->{'ext_trans_id'}) ?
                        $trs->{'ext_trans_id'} : '',
            ...
            $additionalInfo = [
                ...
                'Amount' => round(
                    $currency->convertAmountToFloat($response->get('effective_amount')),
                    $currency->getNum()
                ) . ' ' . $currency->getAlpha3(),
                'Transaction ID' => $transactionId,
            ...
            $transactionType = $this->convertTransactionType($response->get('trans_status'));
    ...
    public function saveIdentifier(\Magento\Sales\Model\Order $order, \Lyranetwork\Payzen\Model\Api\PayzenResponse $response)
    {
        ...
        if ($response->get('identifier') && (
                $response->get('identifier_status') == 'CREATED' /* page_action REGISTER_PAY */ ||
                $response->get('identifier_status') == 'UPDATED' /* page_action REGISTER_UPDATE_PAY */ ||
                $response->get('page_action') == 'ASK_REGISTER_PAY'
        )) {
     ...
     * @param \Lyranetwork\Payzen\Model\Api\PayzenResponse $response
     */
    public function cancelOrder(\Magento\Sales\Model\Order $order, \Lyranetwork\Payzen\Model\Api\PayzenResponse $response)
    {
    ...
    public function addTransaction($payment, $type, $transactionId, $additionalInfo)
    {
        ...
        if ($parentTxn && $parentTxn->getId()) {
            $payment->setTransactionId(
                $this->transactionManager->generateTransactionId($payment, $type, $parentTxn)
            );
